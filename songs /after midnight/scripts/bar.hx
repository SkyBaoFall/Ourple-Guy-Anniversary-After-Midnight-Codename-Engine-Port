...
function onGameOver(_) _.character = 'brookearcade';
function postCreate() {
	FlxG.camera.zoom = 5;
	maxCamZoom *= 6;
	camHUD.alpha = 0;
	for (i in [dad, boyfriend, gf]) members.remove(i);
	for (i in [gf, dad, sparkles, boyfriend, rain, blackScreen, spotlight1, spotlight2]) add(i);
	dad.alpha = 0;
...
function onEvent(_) {
	var name = _.event.name;
	var params = _.event.params;
	//trace('Do Event: ' + name + ' - ' + params);
	switch(name) {
		case 'brookecam':
		...
		case 'endingoff':
			switch(params[0]) {
				case 1:
					...
					FlxTween.tween(camHUD, {alpha: 0}, 1, {ease: FlxEase.sineOut, onComplete: () -> {FlxTween.tween(endingScene, {alpha: 1}, 4, {ease: FlxEase.sineOut});}});
		...
		case 'Change Character':
			switch(params[1]) {
				case 'brookehand':
					...
					boyfriend.animation.callback = (s,n,i)->{
						if (!panelOn) moveCam(true, boyfriend);
					};
					...
					for (i => spr in currentHUD.members) {
						if (i < 2) FlxTween.tween(spr, {alpha: 0}, 3, {ease: FlxEase.sineIn});
						if (i > 3 && i < 7) FlxTween.tween(spr, {alpha: 0}, 3, {ease: FlxEase.sineIn});
					}
					for (i in cpuStrums) {
				...
				case 'prangealt': sparkles.visible = true;
				case 'brookebig':
					boyfriend.cameras = [fakeCam];
		...
		case 'drinking': brookesit.animation.play('drinking', true);
		case 'insidebar':
			...
			moveCam(true);
		case 'limebye': lime.playAnim('bye', true);
		case 'midnCam': 
			ongoing = true;
			allowMoveCam = false;
		...
		case 'bgchange':
			...
			FlxTween.tween(FlxG.camera.flashSprite, {scaleX: -1}, getSecondTimeFromBeat(Conductor.curBeat)/2, {type: 4, ease: FlxEase.cubeInOut, onComplete: (tw)->{
				if (tw.executions == 2) for (i in [trees, front, bg]) i.visible = false;
				if (tw.executions == 4) tw.cancel();
			}});
		case 'insidetrans':
			defaultCamZoom = 1.3;
			ongoing = true;
		...
		case 'Play Animation':
			switch(params[0]) {
				...
				case 'spin': //nothing here cuz use in events
			}
		case 'spotlight':
		...
		case 'end': for (i in [spotlight1, spotlight2, blackScreen]) members.remove(i);
		case 'flicker':
			flicker = !flicker;
		...
		case 'blackfadein': FlxTween.tween(blackScreen, {alpha: 1}, 1, {ease: FlxEase.sineOut});
		case 'blackfadeout': FlxTween.tween(blackScreen, {alpha: 0}, 1, {ease: FlxEase.sineOut});
	}
}
